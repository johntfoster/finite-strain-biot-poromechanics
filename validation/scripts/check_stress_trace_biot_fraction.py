#!/usr/bin/env python3
"""Symbolic (SymPy) check of the closed-form Biot coefficient in Appendix B.
...
``paper/sections/stress_trace_biot_appendix.tex``:
  ...
  * eq. (87) -> eq. (88): implicit differentiation of the scalar mineral
    residual at fixed pore pressure;
  * eq. (88) -> eq. (89): explicit tangent ``dJbar/dJ``;
  * eq. (89) -> eq. (90): the closed-form coefficient ``B``;
  * the proper-fraction rewrite of eq. (90);
  * the reference limit ``B0 = 1 - K/K_s``;
  * the drained path ``B(J,0)``;
  * the Maxwell reciprocity (eq. 83-84) and the compatible mineral PDE
    (eq. 85).
...
def main() -> int:
    ...
    rho, phi, rho0, mass = sp.symbols("rho phi rho0 mass", positive=True)
    residuals = sp.Matrix([
        J * rho * phi - mass,
        sp.log(rho / rho0) + K / (phi_s0 * Ks) * sp.log(J / ap)
        - alpha * p * rho0 / (Ks * rho),
    ])
    state_tangent = -residuals.jacobian([rho, phi]).inv() * residuals.diff(J)
    density_tangent = -K * rho / (phi_s0 * J * (Ks + alpha * p * rho0 / rho))
    fraction_tangent = phi / J * (K / (phi_s0 * (Ks + alpha * p * rho0 / rho)) - 1)
    ...
    zero("two_state_biot", (1 + J * phi / rho * state_tangent[0]).subs(
        {phi: phi_s0 * rho0 / (J * rho)}).subs(rho, rho0 / Jbar) - B_closed)
