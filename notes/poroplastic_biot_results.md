...
- Curated sweep (axial stretch 0.95-0.60, M=0.2, beta=0.4):
  `validation/poroplastic_delta_b.csv` (columns axial_stretch, compression,
  a_p, B_el, B_pl, delta_B).  Generated by running
  `moose_app/test/tests/poroplastic_biot/poroplastic_delta_b.i` at each
  compression; see `scripts/plot_poroplastic_delta_b.py`.
- Figure: `figures/poroplastic_delta_b.png` (viewing) and
  `figures/poroplastic_delta_b.pgf` (manuscript convention, consumed with
  `\input{...}`; Matplotlib PGF backend).  Both written by
  `scripts/plot_poroplastic_delta_b.py`; the PGF backend is configured to the
  repository's TeX engine (`pgf.texsystem = lualatex`; lualatex is installed,
  xelatex/dvipng are not).
