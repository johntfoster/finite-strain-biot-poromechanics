...
2. DONE (manuscript): the comparison entered the paper as subsection
   Sec. 4.6 "Drained sandstone hydrostatics as a compactive target" with
   Figure 7 (`figures/sandstone_comparison.pgf`), generated by
   `scripts/plot_sandstone_comparison.py` (PGF manuscript convention; also
   .png view).  The prose states the provenance (published copy, Table 3),
   shows the constant-modulus model gap, and labels the two calibrated curves
   as NOT predictions of the present model.  `make figures` now regenerates
   all committed figure scripts.
