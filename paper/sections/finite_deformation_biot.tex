...
volume, including the pore space, and is given by
\begin{equation*}
...
fixed pressure \citep[eqs.~(34)--(36)]{fosterxu2025}, giving
\begin{align}
 \mbf P^{\prime}
...
\end{equation}
Our earlier work expresses the same Biot coefficient in terms of the intrinsic
...
respect to pressure gives mineral volume, so
\begin{align*}
 \mbf P
...
preserves the normalized elastic shape \((J^e)^{-1/3}\mbf F^e\), since
\begin{align}
...
Rearranging this transformation defines the double-prime Kirchhoff stress as
\begin{equation*}
...
independent arguments \((J^e,\bar J)\) as
\begin{align}
 \delta W_s
...
\begin{equation*}
 \alpha\equiv 1-\frac{\bar{J} K}{JK_s}.
 % equation-id: eq:finite-drained-pressure-multiplier
\end{equation*}
The multiplier \(\alpha\) is evaluated at the current pressure, rather than
at zero pressure.  Both \(\alpha\) and \(B\) reduce to \(B_0=1-K/K_s\) in the
...
\begin{gather}
 ...
 =\bs\sigma^{\prime\prime}_d+(B-\alpha)p\mbf I,
 \notag
 % equation-id: eq:double-prime-drained-pressure-split
\\
 \boxed{\bs\sigma=\bs\sigma^{\prime\prime}_d-\alpha p\mbf I},
 \label{eq:total-stress-drained-pressure-split}\\
 \bs\sigma^{\prime}
 =\bs\sigma^{\prime\prime}_d+(1-\alpha)p\mbf I.
...
At fixed deformation and plastic state, \(\alpha\) varies with pressure
...
\begin{equation*}
 B=\alpha+p\left.\pder{\alpha}{p}\right|_{\mbf F,\mbf F^p}.
 % equation-id: eq:biot-tangent-finite-pressure-multiplier
\end{equation*}
Thus \(\alpha\) measures a finite stress difference from zero pressure,
...
connected to zero pressure, \(\alpha\) is the pressure average of \(B\)
...
Biot coefficient.  In the present notation, that reversible Biot coefficient
corresponds to \(B\); \(\alpha\) instead compares stresses over a finite
...
Makhnenko and Labuz use \(\alpha\) for the reversible Biot coefficient and
\(B\) for Skempton's coefficient; here \(\alpha\) instead denotes the finite
pressure multiplier and \(B\) denotes the reversible Biot coefficient.

For the elastic specialization, \(\alpha\) is Gajo's pressure factor
...
Biot coefficient through the pressure Legendre transformation and evaluates it
...
mean stress.  The yield condition, flow rule, and complementarity conditions are
\begin{align}
 f&=q-Mp^{\prime}-c\leq0,
 \notag
