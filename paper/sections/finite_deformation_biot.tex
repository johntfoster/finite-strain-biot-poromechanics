...
\end{equation}
The factor \(\bar J^{-1/3}\) removes the volumetric stretch from
\(\bar{\mbf F}^e\).  The resulting isochoric deformation
\(\bar J^{-1/3}\bar{\mbf F}^e\) is unchanged by this variation through the
same cancellation shown in \eqref{eq:elastic-shape-variation-expansion}.
...
in the independent arguments \((J^e,\bar J)\).  On the zero-pressure
equilibrium branch, substitution of
...
can exist; at fixed \(J^e\), we follow the branch from this root.
...
Drained skeleton moduli are typically calibrated in tests conducted at zero
gauge pore pressure.  At \(p=0\), the final term in
\eqref{eq:matched-logarithmic-double-prime-stress} vanishes, so the measured
response depends only on \(G\) and \(K\).  At nonzero pore pressure, the
mineral state and \(B\) determine the additional poromechanical contribution.
...
standard multiplicative plasticity \citep{yamakawaetal2021}.  The stress is
single-prime because \(W^{\prime}\) is the fixed-intrinsic-density potential.
Its deformation derivative, evaluated in the pressure representation at fixed
\(p\), is \(\mbf P^{\prime}\) by
\eqref{eq:single-prime-energy-conjugate}.
...
the smooth cone branch \(q>0\).  To permit isotropic hardening, let
\(\kappa^p\) be the accumulated plastic multiplier and prescribe
\begin{align}
 c(\kappa^p)&=c_0+H\kappa^p,
 \qquad c_0\geq0,\quad H\geq0,
 \label{eq:plastic-isotropic-hardening}\\
 \dot{\kappa}^p&=\dot\gamma,
 \qquad \kappa^p(0)=0.
...
perfect plasticity.  Taking the trace of the flow rule gives
\begin{equation}
...
\begin{equation}
 \kappa^p_{n+1}=\kappa^p_n+\Delta\gamma.
