...
\begin{equation}
 B=1-\frac{1}{v_{s0}}
 \left.\frac{\partial\bar v_s}{\partial J_s}\right|_{p_E,T,\ldots},
 \label{eq:nonlinear-biot}
\end{equation}
where \(v_{s0}\) is the intrinsic skeleton reference specific volume and
\(p_E\) is equivalent pore pressure.  The qualifier ``fixed \(p_E\)'' is a
...
Piola--Kirchhoff stress evaluated at fixed \(p_E\).  Because \(B\) is a state
variable, the consistent Newton derivative of \cref{eq:reference-biot-stress}
contains derivatives of \(B\), \(p_E\), \(J_s\), and \(\mbf F_s^{-T}\).
