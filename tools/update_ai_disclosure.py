...
def render_markdown(registry: dict[str, Any], first: dict[str, str], end: dict[str, str]) -> str:
    ...
    model_record = registry["disclosure"].get("model_record", "Model versions remain unknown where request metadata is unavailable.")
    session_record = registry["disclosure"].get("session_coverage_record")
    session_link = f"[Session coverage and model evidence]({Path(session_record).name})" if session_record else "No supplemental session inventory is registered."
    if end["kind"] == "final":
    ...
    development_record = registry["disclosure"].get("development_record", "")
    codespace_url = registry["project"].get("codespace_url")
    laboratory = (
        f"The configured [GitHub Codespace]({codespace_url}) serves as a reproducible "
        "computational laboratory, providing the pinned software environment, "
        "MOOSE implementation, verification tests, and scripts needed to rerun "
        "the calculations and regenerate the figures and manuscript."
        if codespace_url else ""
    )
...
- Model versions: {model_record}
- Supplemental development record: {session_link}
...
def render_latex(registry: dict[str, Any], first: dict[str, str], end: dict[str, str]) -> str:
    ...
    model_record = latex_escape(registry["disclosure"].get("model_record", ""))
    if end["kind"] == "final":
    ...
    uses = latex_escape("; ".join(registry["uses"])).replace("LaTeX", r"\LaTeX{}")
    development_record = latex_escape(registry["disclosure"].get("development_record", ""))
    public_url = registry["project"].get("public_repository_url")
    codespace_url = registry["project"].get("codespace_url")
    laboratory = (
        "The configured \\href{" + codespace_url + "}{GitHub Codespace} serves "
        "as a reproducible computational laboratory, providing the pinned "
        "software environment, MOOSE implementation, verification tests, and "
        "scripts needed to rerun the calculations and regenerate the figures "
        "and manuscript."
        if codespace_url else ""
    )
    public_record = (
        "The repository at \\url{" + public_url + "} is available for inspection "
        "of the manuscript's changes and development history."
        if public_url else ""
    )
    return f"""% Generated by tools/update_ai_disclosure.py; edit provenance/ai-use.yml instead.
